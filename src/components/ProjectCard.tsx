  ...
  return (
    ...
    >
      ...
      <div className={`md:w-2/5 ${mediaBg} flex items-center justify-center p-5 md:p-7`}>
        <div className="relative w-full aspect-[4/3] rounded-xl overflow-hidden shadow-sm ring-1 ring-blue-500/30 flex items-center justify-center bg-gray-700/50">
          {image ? (
            <Image
              src={image.src}
              alt={image.alt}
            ...
            <div className="transition-transform duration-500 group-hover:scale-105 flex flex-col items-center gap-2 text-blue-400/60">
      ...
      <div className="md:w-3/5 p-6 flex flex-col justify-between gap-4">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => (
              <span key={tag} className="px-2 py-0.5 text-xs font-medium bg-gray-700 text-blue-300 border border-blue-700 rounded-full">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          ...
          {writeup && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-lg border border-blue-500 text-blue-400 hover:bg-blue-900/20 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
            >
              Write-up
            </a>
          )}
          {!github && !demo && !writeup && (
            <span className="px-4 py-2 text-sm text-gray-500 italic">Links coming soon</span>
          )}
        </div>
      </div>
    </div>
